...
module.exports = {
    ...
    execute(message, args) {
        ...
        message.guild.channels.cache.forEach((channel) => {
            if (channel.name == 'report-'+message.author.username.toLowerCase() + "-" + message.author.discriminator) {

                message.channel.send("You already made a report");
        ...
        if (bool == true) return;
        message.guild.channels.create('report-'+message.author.username + "-" + message.author.discriminator, "text").then((createdChan) => { // Maak kanaal
            createdChan.setParent(categoryId).then((settedParent) => { // Zet kanaal in category.
                ...
                message.reply(`report has been created, go to ${createdChan}`)

                createdChan.send(`${message.author} Please reply with you\'re report reason.`)
            })
        });
